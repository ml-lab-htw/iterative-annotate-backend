...
def main(name):
    ...
    for bundle_id, ground_truth in bundles.items():
        inference_path = Path(f"{base_path}/{name}/{bundle_id}/_inference_results.json")
        with open(inference_path, "r") as f:
            json_data = json.load(f)["inference"]

        inference_data = json_data["result"]
        bundle_nbr = extract_bundle_number(bundle_id)

        metrics = evaluate_bundle(ground_truth, inference_data)

        print(f"{bundle_nbr}. - {metrics['average_precision']}")
        data_group.append({
            "bundle_id": bundle_nbr,
            "inference_duration": json_data["duration"],
            "boxes": {
                "inference": [[box["box"] for box in anno["annotations"]] for anno in inference_data],
                "ground_truth": [truth["annotations"] for truth in ground_truth],
            },
            "eval": metrics
        })
...
if __name__ == "__main__":
    run_id = 1
    run_folder = "n_10"
    extracted = main(run_folder)

    # Create the JSON file spath
    folder = Path(f"{base_path}/{run_folder}/_eval/")
    folder.mkdir(parents=True, exist_ok=True)

    # Write the JSON file
    with open(folder / "inference.json", 'w') as json_file:
