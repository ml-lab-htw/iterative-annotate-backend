...
def plot_metric(steps, metric_values, metric_name, run_id, colors, custom_labels):
    plt.figure(figsize=(12, 6))
    plt.tight_layout()

    for data_type, values in metric_values.items():
        plt.plot(steps, values, label=custom_labels[data_type], marker='o', color=colors[data_type])
    ...
    output_path = os.path.join(base_path, script_path, f"pdf_export/{metric_name}_{run_id}.pdf")
...
def main(run_id):
    ...
    for metric_name in ['precision', 'recall', 'iou', 'f1']:
        plot_metric(steps, {dt: metric_values[dt][metric_name] for dt in metric_values}, metric_name, run_id, colors, custom_labels)
