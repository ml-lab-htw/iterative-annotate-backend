...
def main(name):
    ...
    for bundle_id, ground_truth in bundles.items():
        manual_path = Path(f"{base_path}/{name}/{bundle_id}/_manual_labeling.json")
        with open(manual_path, "r") as f:
            json_data = json.load(f)

        manual_data = json_data["images"]
        bundle_nbr = extract_bundle_number(bundle_id)

        metrics = evaluate_bundle(ground_truth, manual_data)
...
if __name__ == "__main__":
    ...
    # Write the JSON file
    with open(folder / "manual.json", 'w') as json_file:
